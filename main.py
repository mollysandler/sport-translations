...
class DynamicSpeakerTranslator:
    ...
    def _initialize_services(self):
        ...
        if self.tts_config.qwen_enable:
            print("   🗣️  Loading Qwen3-TTS (local voice cloning)...")
            qwen_device = self.tts_config.qwen_device  # 'cpu' is safest on macOS
            qwen_model_id = self.tts_config.qwen_model_id
        ...
        self.xtts_cloner = None
        if self.tts_config.xtts_enable:
            print("   🧬 Loading XTTS (free local voice cloning backup)...")
            xtts_device = self.tts_config.xtts_device
            xtts_model = self.tts_config.xtts_model_id
...
def _build_arg_parser():
    ...
    p.add_argument("--tgt", "--target", dest="target_lang", default="es", help="Target language (default: es)")
    ...
    # Auth
    p.add_argument("--hf-token", default=os.getenv("HUGGING_FACE_TOKEN"), help="Hugging Face token (default: env:HUGGING_FACE_TOKEN)")
    ...
    # TTS / cloning backends (formerly env vars)
    ...
    p.add_argument("--qwen-tts-device", default="cpu", help='Qwen device (default: "cpu")')
    ...
    # Speaker merge tuning (formerly env vars)
...
def main():
    ...
    output = translator.translate_video_streaming(args.video, hf_token=args.hf_token)
