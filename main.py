...
class SmartVoiceManager:
    ...
    def analyze_speaker_characteristics(
        self, audio_sample: torch.Tensor, speaker_id: str, sample_rate: int = 16000
    ) -> VoiceProfile:
        """
        ...
        print(
            f"      Gender: {gender} | Pitch: {avg_pitch:.0f}Hz | Rate: {speaking_rate:.1f}w/s"
        )
    ...
    def _match_best_voice(
        self, gender: str, avg_pitch: float, pitch_range: float
    ) -> str:
        """
        ...
        print(
            f"      ✅ Matched to: {best_match[:8]}... ({self.available_voices[best_match]['style']})"
        )
    ...
    def assign_voices(
        ...
        for speaker_id, profile in sorted(self.assigned_voices.items()):
            ...
            print(
                f"   {speaker_id}: {profile.gender} | {profile.avg_pitch:.0f}Hz | {voice_style}"
            )

        return {sid: profile.voice_id for sid, profile in self.assigned_voices.items()}

    def _analyze_speaker_safe(
        self, audio_tensor, speaker_id: str, sample_rate: int
    ) -> "VoiceProfile":
        """
...
class QwenLocalVoiceCloner:
    ...
    def __init__(
        self, model_id: str = "Qwen/Qwen3-TTS-12Hz-0.6B-Base", device: str = "cpu"
    ):
...
class XTTSLocalVoiceCloner:
    ...
    def __init__(
        ...
        try:
            ...
            try:
                self.output_sr = int(
                    getattr(self.tts.synthesizer, "output_sample_rate", 24000)
                )
            except Exception:
                self.output_sr = 24000
            self.available = True
    ...
    def synthesize(
        self, text: str, target_lang_iso: str, ref_wav_bytes: bytes
    ) -> Optional[bytes]:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as f:
                ...
                wav = self.tts.tts(
                    text=text, speaker_wav=f.name, language=target_lang_iso
                )
...
class DynamicSpeakerTranslator:
    ...
    def _initialize_services(self):
        ...
        self.whisper = WhisperModel(
            model_size, device="cuda", compute_type="float16", num_workers=2
        )
    ...
    def translate_video_streaming(
        ...
        playback_thread = threading.Thread(
            target=self._playback_thread, args=(buffer_count,), daemon=False
        )
    ...
    def _create_speaker_voice_clones(
        self, audio: torch.Tensor, segments: List[SpeakerSegment], sample_rate: int
    ) -> Dict[str, str]:
        """
        ...
        ref_min_score = float(os.getenv("CLONE_REF_MIN_SCORE", "0.03"))  # noqa: F841
        ref_min_ms = int(os.getenv("CLONE_REF_MIN_MS", "800"))
        ref_max_seg_sec = float(os.getenv("CLONE_REF_MAX_SEG_SEC", "3.0"))
        ref_debug = os.getenv("CLONE_REF_DEBUG", "0") == "1"  # noqa: F841
        save_ref = os.getenv("CLONE_REF_SAVE", "0") == "1"  # noqa: F841
        ...
        if use_clean_ref and spkrec is not None:
            by_spk = {
                sid: [s for s in segments if s.speaker_id == sid]
        ...
        for speaker_id in unique_speakers:
            ...
            target = float(
                os.getenv(
                    "CLONE_REF_SECONDS", str(self.tts_config.clone_ref_target_sec)
                )
            )
            min_required = float(
                os.getenv("CLONE_MIN_SECONDS", str(self.tts_config.clone_ref_min_sec))
            )
            ...
            if not collected:
                print(
                    f"   ⚠️  {speaker_id}: insufficient audio for cloning; will rely on fallback voices."
                )
                continue
            ...
            if float(total) < min_required:
                print(
                    f"   ⚠️  {speaker_id}: only {total:.1f}s ref audio (<{min_required:.1f}s). Skipping local cloning; will use ElevenLabs."
                )
                continue
            ...
            try:
                ref_whisper_lang = (
                    self.source_lang if self.source_lang != "auto" else None
                )
        ...
        for speaker_id in unique_speakers:
            ...
            has_ref = (speaker_id in self.speaker_ref_wav) and (
                self.speaker_ref_sec.get(speaker_id, 0.0)
                >= float(
                    os.getenv(
                        "CLONE_MIN_SECONDS", str(self.tts_config.clone_ref_min_sec)
                    )
                )
            )
            ...
            if has_qwen:
                print(
                    f"   ✅ {speaker_id}: using Qwen voice clone (backup stock voice {fallback_map[speaker_id][:8]}...)"
                )
            elif has_xtts:
                print(
                    f"   ✅ {speaker_id}: using XTTS voice clone (backup stock voice {fallback_map[speaker_id][:8]}...)"
                )
            else:
                print(
                    f"   ✅ {speaker_id}: using fallback stock voice ({fallback_map[speaker_id][:8]}...)"
                )
    ...
    def _print_segments(self, label, segs):
        ...
        for s in segs:
            print(
                f"{s.speaker_id:10s} {s.start_ms:6d}-{s.end_ms:6d}  ({(s.end_ms - s.start_ms)}ms)"
            )
            # overlap check
        overlaps = 0
    ...
    def _make_exclusive_turns(
        ...
        time_to_ends = {}
        for seg in segs:
            time_to_starts.setdefault(seg.start_ms, []).append(
                (seg.speaker_id, seg.start_ms)
            )
        ...
        for spk, s, e, _solo_ms in merged2:
            ...
            out.append(
                SpeakerSegment(spk, s, e + PAD_MS, s / 1000.0, (e + PAD_MS) / 1000.0)
            )
    ...
    def _process_segments_parallel(
        self, audio: torch.Tensor, segments: List[SpeakerSegment]
    ):
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for idx, seg in enumerate(segments, 1):
                    future = executor.submit(
                        self._process_single_segment, idx, seg, audio
                    )
                ...
                for future in as_completed(future_to_segment):
                    ...
                    try:
                        ...
                        print(
                            f"⚡ Completed {processed_count}/{len(segments)} ({progress:.0f}%) | {elapsed:.1f}s"
                        )
            ...
            speedup = (
                (audio.shape[1] / self.sample_rate) / total_time
                if total_time > 0
                else 0.0
            )
            print(
    ...
    def _tts_bytes_to_wav(
        self, audio_bytes: bytes, output_sr: int = 24000
    ) -> Tuple[bytes, int]:
        """
    ...
    def _process_single_segment(
        ...
        try:
            ...
            text_parts = [
                s.text.strip() for s in segments_whisper if s.text and s.text.strip()
            ]
            if not text_parts:
                print(
                    f"   ⚠️  No ASR text for {seg.speaker_id} (segment {idx}, {seg.end_ms - seg.start_ms}ms) — skipping"
                )
                return None
            ...
            def _fit_tts_to_original(
                wav_bytes: bytes, tts_ms: int, orig_ms: int
            ) -> Tuple[bytes, int]:
                """
                ...
                MAX_RATE_UP = float(
                    os.getenv("TTS_MAX_RATE_UP", "1.25")
                )  # up to 25% faster
                MAX_RATE_DOWN = float(  # noqa: F841
                    os.getenv("TTS_MAX_RATE_DOWN", "0.90")
                )  # allow ~10% slower if too short
                ...
                try:
                    ...
                    if len(a) > orig_ms:
                        ...
                        a = a + AudioSegment.silent(
                            duration=(orig_ms - len(a)), frame_rate=a.frame_rate
                        )
                    bio = io.BytesIO()
                ...
                if tts_ms > orig_ms:
                    ...
                    try:
                        ...
                        a = (
                            AudioSegment.from_wav(io.BytesIO(wav2))
                ...
                # If TTS is shorter, pad (optionally slow down slightly, but padding is safer)
                    ...
                    a = (
                        AudioSegment.from_wav(io.BytesIO(wav_bytes))
                        .set_frame_rate(24000)
                        .set_channels(1)
                    )
            ...
            xtts_max_chars = int(
                os.getenv(
                    "XTTS_MAX_CHARS",
                    str(lang_default_limits.get(self.target_lang, 220)),
                )
            )
            ...
            if self.use_voice_cloning:
                ...
                has_ref_audio = (seg.speaker_id in self.speaker_ref_wav) and (
                    ref_seconds >= min_required
                )
                ...
                if has_ref_audio:
                    ...
                    if qwen_lang_ok:
                        if seg.speaker_id not in self.speaker_qwen_prompt:
                            with self._qwen_prompt_lock:
                                if seg.speaker_id not in self.speaker_qwen_prompt:
                                    try:
                                        ref_wav = self.speaker_ref_wav.get(
                                            seg.speaker_id
                                        )
                                        ref_np, ref_sr = (
                                            _audio_np_from_wav_bytes(ref_wav)
                                            if ref_wav
                                            else (None, None)
                                        )
                                        ...
                                        if prompt is not None:
                                            self.speaker_qwen_prompt[seg.speaker_id] = (
                                                prompt
                                            )
                                            print(
                                                f"   ✅ {seg.speaker_id}: Qwen prompt created (lazy)"
                                            )
                                    except Exception as e:
                                        print(
                                            f"   ⚠️  Qwen prompt creation failed for {seg.speaker_id}: {e}"
                                        )

                        prompt = self.speaker_qwen_prompt.get(seg.speaker_id)
                        if prompt is not None:
                            try:
                                qwen_bytes = self.qwen_cloner.synthesize(
                                    translated_text, self.target_lang, prompt
                                )
                                if qwen_bytes:
                                    wav_bytes, duration_ms = self._tts_bytes_to_wav(
                                        qwen_bytes, output_sr=24000
                                    )
                                    wav_bytes, duration_ms = _fit_tts_to_original(
                                        wav_bytes, duration_ms, orig_ms
                                    )
                            except Exception as e:
                                print(
                                    f"   ⚠️  Qwen TTS failed for {seg.speaker_id} (segment {idx}): {e}"
                                )
                    ...
                    if wav_bytes is None and self.tts_config.xtts_enable:
                        if (
                            ...
                            and self.xtts_cloner.supports_lang(self.target_lang)
                        ):
                            try:
                                ...
                                chunks = _chunk_text_for_tts(
                                    translated_text, xtts_max_chars
                                )
                                xtts_wavs: List[bytes] = []
                                for c in chunks:
                                    xtts_bytes = self.xtts_cloner.synthesize(
                                        c, self.target_lang, ref_wav
                                    )
                                    if not xtts_bytes:
                                        raise ValueError("XTTS returned empty bytes")
                                    w, _dur = self._tts_bytes_to_wav(
                                        xtts_bytes, output_sr=24000
                                    )
                                    xtts_wavs.append(w)

                                wav_bytes = _concat_wav_bytes(
                                    xtts_wavs, silence_ms=xtts_silence_ms
                                )
                                wav_bytes, duration_ms = self._tts_bytes_to_wav(
                                    wav_bytes, output_sr=24000
                                )
                                wav_bytes, duration_ms = _fit_tts_to_original(
                                    wav_bytes, duration_ms, orig_ms
                                )
                                ...
                                print(
                                    f"   ⚠️  XTTS failed for {seg.speaker_id} (segment {idx}): {e}"
                                )
                                wav_bytes = None
                                duration_ms = None
            ...
            if wav_bytes is None:
                ...
                if not voice_id:
                    print(
                        f"   ⚠️  No voice_id for {seg.speaker_id} (segment {idx}) — skipping"
                    )
                    return None
        ...
        try:
            ...
            while self.playback_queue.qsize() < buffer_count:
                ...
                if (
                    ...
                    and self.playback_queue.qsize() > 0
                ):
                    break
            ...
            while (
                not self.processing_complete.is_set() or not self.playback_queue.empty()
            ):
                try:
                    ...
                    if pause_ms > 0:
                        play(
                            AudioSegment.silent(
                                duration=int(pause_ms), frame_rate=24000
                            )
                        )

                    print(
                        f"📊 [{segment.speaker_id}] {segment.translated_text[:60]}..."
                    )
                    play(audio_seg)
    ...
    def _compose_audio(self, segments, total_duration_sec):
        ...
        if not segments:
            return AudioSegment.silent(
                duration=int(total_duration_sec * 1000), frame_rate=OUTPUT_SR
            )
    ...
    def translate_audio_file_stream(
        ...
        - yields NDJSON-friendly dict events as segments become available
        - keeps existing translate_audio_file_no_playback intact
        Events:
          {type:"start", total_duration_sec, buffer_duration_sec}
          {type:"buffering", buffered_until_sec, buffered_segments, buffer_target_sec}
          {type:"segment", caption:{...}}  (caption matches your frontend shape)
          {type:"final", audio_base64, captions}
          {type:"error", message}
        """
    ...
    def _assign_chunk_voices(
        ...
        def _apply_match(speaker_id, data, session_spk):
            ...
            if session_spk is not None:
                ...
                if data["embedding"] is not None and session_spk in session_embeddings:
                    ...
                    session_embeddings[session_spk] = (
                        0.7 * old + 0.3 * data["embedding"]
                    )
            else:
                ...
                print(
                    f"   🆕  {speaker_id}: pitch={data['avg_pitch']:.0f}Hz → "
        ...
        if not is_multi_speaker:
            ...
            for speaker_id in speakers_with_data:
                ...
                if session_spk:
                    print(
                        f"   ♻️  {speaker_id}: pitch={data['avg_pitch']:.0f}Hz → "
            ...
            if len(spk_list) == 2:
                ...
                dominant_diar = max(
                    spk_list, key=lambda s: speaker_data[s]["seg_count"]
                )
            ...
            if dom_spk:
                print(
                    f"   ♻️  {dominant_diar}: pitch={dom_data['avg_pitch']:.0f}Hz → "
    ...
    def _get_live_voice_for_chunk(
        ...
        if best_spk and best_dist <= PITCH_MATCH_HZ:
            print(
                f"   ♻️  Live chunk: pitch={avg_pitch:.0f}Hz → reusing {best_spk} (dist={best_dist:.0f}Hz)"
            )
        ...
        print(
            f"   🆕  Live chunk: pitch={avg_pitch:.0f}Hz → new voice {voice_id[:8]}... ({spk_id})"
        )
        return voice_id
        ...
        for start_sec, end_sec, _ in speech_items:
            ...
            if dur < MIN_PITCH_DURATION:
                ...
                if last_spk and last_spk in session_state["speaker_voice_ids"]:
                    print(
                        f"   ⏩  [{start_sec:.1f}-{end_sec:.1f}s] short ({dur:.1f}s) → inherit {last_spk}"
                    )
                    results.append(
                        (last_spk, session_state["speaker_voice_ids"][last_spk])
                    )
                    continue
            ...
            if dur >= 0.3:
                try:
                    seg_np = (
                        seg_audio.squeeze().detach().cpu().numpy().astype(np.float32)
                    )
                    try:
            ...
            if best_spk and best_dist <= PITCH_MATCH_HZ:
                ...
                session_state["speaker_pitches"][best_spk] = (
                    0.7 * old_pitch + 0.3 * avg_pitch
                )
                print(
                    f"   ♻️  [{start_sec:.1f}-{end_sec:.1f}s] pitch={avg_pitch:.0f}Hz → {best_spk} (Δ{best_dist:.0f}Hz)"
                )
                session_state["last_speaker_id"] = best_spk
                results.append((best_spk, session_state["speaker_voice_ids"][best_spk]))
            else:
                voice_id = self.voice_manager._match_best_voice(
                    gender, avg_pitch, pitch_range
                )
                spk_id = f"SPEAKER_{len(session_state['speaker_voice_ids']):02d}"
                session_state["speaker_voice_ids"][spk_id] = voice_id
                session_state["speaker_pitches"][spk_id] = avg_pitch
                print(
                    f"   🆕  [{start_sec:.1f}-{end_sec:.1f}s] pitch={avg_pitch:.0f}Hz → new {spk_id}"
                )
    ...
    def translate_chunk_stream(
        ...
        if overlap_duration_sec > 0 and speech_items:
            ...
            for start, end, text in speech_items:
                ...
                if start < overlap_duration_sec - 0.1:
                    print(
                        f"   🗑️  Overlap dedup: dropping [{start:.1f}-{end:.1f}s] (starts in overlap)"
                    )
                    continue
        ...
        if not speech_items:
            print(
                f"   ℹ️  Live chunk {chunk_duration_ms}ms: no speech detected by Whisper VAD"
            )
            session_state["chunk_offset_ms"] = (
                chunk_offset_ms + chunk_duration_ms - int(overlap_duration_sec * 1000)
            )
            return

        print(
            f"   ✅ Live chunk {chunk_duration_ms}ms: {len(speech_items)} Whisper segment(s)"
        )

        # ── Per-segment speaker assignment (pitch-based, cross-chunk continuity) ──
        speaker_assignments = self._assign_live_speakers(
            audio, speech_items, sr, session_state
        )
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            ordered_futures = []
            for (start, end, text), (speaker_id, voice_id) in zip(
                speech_items, speaker_assignments
            ):
            ...
            for future, speaker_id in ordered_futures:
                completed_count += 1
                try:
                    start_sec, end_sec, original_text, translated_text, mp3_bytes = (
                        future.result()
                    )
                except Exception as e:
...
def _build_arg_parser():
    ...
    p.add_argument(
        "--buffer",
        dest="buffer_duration_sec",
        type=int,
        default=30,
        help="Buffer seconds before playback starts (default: 30)",
    )
    p.add_argument(
        "--workers",
        dest="max_workers",
    ...
    vc.add_argument(
        "--use-voice-cloning",
        dest="use_voice_cloning",
        action="store_true",
        default=True,
        help="Enable voice cloning (default: enabled)",
    )
    vc.add_argument(
        "--no-voice-cloning",
        dest="use_voice_cloning",
        action="store_false",
        help="Disable voice cloning; use pleasant stock voices",
    )

    # TTS / cloning backends
    p.add_argument(
        "--tts-backend", default="qwen", help='TTS backend label (default: "qwen")'
    )
    p.add_argument(
        "--qwen-tts-enable",
        type=int,
        default=1,
        help="Enable Qwen local TTS/clone (default: 1)",
    )
    p.add_argument(
        "--qwen-tts-model-id",
        default="Qwen/Qwen3-TTS-12Hz-0.6B-Base",
        help="Qwen model id (default: Qwen/Qwen3-TTS-12Hz-0.6B-Base)",
    )
    p.add_argument(
        "--qwen-tts-device", default="mps", help='Qwen device (default: "mps")'
    )
    p.add_argument(
        "--xtts-enable",
        type=int,
        default=1,
        help="Enable XTTS local TTS/clone (default: 1)",
    )
    p.add_argument(
        "--xtts-model-id",
        default="tts_models/multilingual/multi-dataset/xtts_v2",
        help="XTTS model id (default: tts_models/multilingual/multi-dataset/xtts_v2)",
    )
    p.add_argument("--xtts-device", default="cpu", help='XTTS device (default: "cpu")')

    # Speaker merge tuning
    p.add_argument(
        "--speaker-merge-enable",
        type=int,
        default=1,
        help="Enable speaker merge/consolidation (default: 1)",
    )
    p.add_argument(
        "--speaker-merge-sim",
        type=float,
        default=0.74,
        help="Cosine similarity threshold for merging speakers (default: 0.74)",
    )
    p.add_argument(
        "--speaker-tiny-total-ms",
        type=int,
        default=6000,
        help="Speakers with < this total ms get absorbed (default: 6000)",
    )
    p.add_argument(
        "--speaker-emb-min-chunk-ms",
        type=int,
        default=250,
        help="Minimum chunk length for speaker embeddings (default: 250)",
    )
    p.add_argument(
        "--speaker-merge-ref-sec",
        type=float,
        default=20.0,
        help="Reference audio seconds per speaker for embedding (default: 20)",
    )
